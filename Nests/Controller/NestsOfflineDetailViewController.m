//
//  NestsOfflineDetailViewController.m
//  Nests
//
...
@interface NestsOfflineDetailViewController ()<UITableViewDataSource, UITableViewDelegate,MAHeaderViewDelegate>
{
...
@implementation NestsOfflineDetailViewController
...
- (NSString *)cellDetailTextForItem:(MAOfflineItem *)item
{
    ...
    {
        return [NSString stringWithFormat:@"大小:%lld", item.size];
    }
...
- (UIButton *)buttonWithImage:(UIImage *)image tag:(NSUInteger)tag
{
    ...
    if (tag==kTagPauseButton) {
         [button setTitle:@"暂停" forState:UIControlStateNormal];
        [button.titleLabel setFont:[UIFont systemFontOfSize:13]];
        [button setTitleColor:[UIColor colorWithRed:0.99 green:0.74 blue:0.0 alpha:1.0] forState:UIControlStateNormal];
    }
    else
    {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (section < self.sectionTitles.count)
    {
        ...
        lb.text = theTitle;
        lb.textColor = [UIColor lightTextColor];
...
- (UIImage *)downloadImage
{
    if (_download == nil)
    {
        _download = [UIImage imageNamed:@"zone_download"];
    }
    return _download;
}
...
- (UIImage *)deleteImage
{
    if (_delete == nil)
    {
        _delete = [UIImage imageNamed:@"zone_delte"];
    }
    return _delete;
}
...
//    [self.tableView showViewBorder:[UIColor greenColor] borderWidth:1.0];
//    [self.view showViewBorder:[UIColor redColor] borderWidth:1.0];
